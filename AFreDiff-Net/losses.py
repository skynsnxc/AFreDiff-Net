...
class BCEDiceLoss_newversion(nn.Module):
    ...
    def forward(self, input, target):
        # _, input = input  # 运行isic2017 ph2 的train需要注释
...
class GT_BceDiceLoss_new(nn.Module):
    ...
    def forward(self, pre, out, target):
        ...
        gt_loss = self.bcedice(gt_pre4, target) * 0.1 + self.bcedice(gt_pre3, target) * 0.2 + self.bcedice(gt_pre2,
            ...
            gt_pre1, target) * 0.4 + self.bcedice(gt_pre0, target) * 0.5
        return bcediceloss + gt_loss
...
class GT_BceDiceLoss_new1(nn.Module):
    ...
    def forward(self, pre, out, target):
        ...
        return bcediceloss + gt_loss, self.bcedice(gt_pre4, target), self.bcedice(gt_pre3, target), self.bcedice(
            gt_pre2, target), self.bcedice(gt_pre1, target), self.bcedice(gt_pre0, target)
