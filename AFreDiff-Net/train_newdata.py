...
import yaml
from albumentations import Resize, Rotate, VerticalFlip, HorizontalFlip
...
ARCH_NAMES = archs_AFreDiff-Net.__all__
...
def parse_args():
    ...
    # model
    # TODO:这个default="AFreDiff-Net_Net"要和archs_AFreDiff-Net.py中 AFreDiff-Net_Net 类的名字保持一致
    parser.add_argument('--arch', '-a', metavar='ARCH', default="AFreDiff-NetBlock_Synergy")
    ...
    # loss
    # TODO:这个loss可以选，在lossed.py里面
    parser.add_argument('--loss', default='GT_BceDiceLoss',
                        choices=LOSS_NAMES,
                        help='loss: ' +
                             ' | '.join(LOSS_NAMES) +
                             ' (default: BCEDiceLoss)')
    ...
    # dataset
    # TODO:default='ph2',数据集的名称，换的话要改名；default='.bmp'是图片的格式，不一样的话一定要改
    ...
    # parser.add_argument('--dataset', default='CVC-ClinicDB',
    #                        help='dataset name')
    ...
    parser.add_argument('--root', default='E:/AFreDiff-Net-Net-main', help='root path to datasets')
...
def train(config, train_loader, model, criterion, optimizer):
    ...
    for input, target, _ in train_loader:
        ...
        #   preds.append(output1)
        ...
        elif config['loss'] == 'BCEDiceLossUNEXT' or config['arch'] == 'AttU_Net' or config['arch'] == 'R2U_Net' or \
                config['arch'] == 'U_Net':
            out = model(input)
            ...
            # TODO:别看它很多的elif 都进不去 ，最终是进这里的。用的loss是我们在parse_args设置的loss。
            # TODO:它这个model返回的实际上是一个元组+out（因为它设置inference_mode=False，看archs_ums.py里的模型就知道了）
            # TODO:这个元祖不知道有什么用，但必须要把元祖和out分开，因为criterion，iou_score弄不了元祖
            # TODO:所以要小修改 losses 中的 BCEDiceLoss_newversion，和 metrics.py的 iou_score
            # TODO:加上   _,input = input   ,   _, output = output 就行    看看就知道了。

            # TODO:唉，当然也可以model(input, inference_mode=True)这样只返回一个out。
            output = model(input)
            loss = criterion(output, target)
            # TODO: iou, dice是两个图像分割的评估指标，越大越好
            iou, dice = iou_score(output, target)
    ...
    with torch.no_grad():
        ...
        for input, target, _ in val_loader:
            ...
            if config['loss'] == 'GT_BceDiceLoss' or config['loss'] == 'GT_BceDiceLoss_new':
                ...
                preds.append(out)
            # TODO:都进不去，只进这里，用的loss是我们在parse_args设置的loss。
            else:
                gts.append(target.squeeze(1).cpu().detach().numpy())
                output = model(input)
                loss = 0
...
def main():
    ...
    # create model
    model = archs_AFreDiff-Net.__dict__[config['arch']](config['num_classes'],
    ...
    # 这个是用于做泛化实验，针对KVASIR-seg的
    train_img_ids = glob(os.path.join(config['root'], config['dataset'], 'train', 'images', '*' + config['img_ext']))
    val_img_ids = glob(os.path.join(config['root'], config['dataset'], 'val', 'images', '*' + config['img_ext']))
    ...
    # TODO:路径必须正确 有个小细节 要去Dataset里看一看！！！
